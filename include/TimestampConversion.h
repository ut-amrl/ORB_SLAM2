...
#ifndef ORB_SLAM2_TIMESTAMPCONVERSION_H
#define ORB_SLAM2_TIMESTAMPCONVERSION_H
...
namespace ORB_SLAM2 {
    ...
    static inline double toDoubleInSeconds(const std::pair<uint32_t, uint32_t> &timestamp_pair) {
        return static_cast<double>(timestamp_pair.first) + kSecondsInNsec * static_cast<double>(timestamp_pair.second);
    }

    static inline std::pair<uint32_t, uint32_t> toTimestampPair(const double &timestamp_sec) {
...
#endif //ORB_SLAM2_TIMESTAMPCONVERSION_H
